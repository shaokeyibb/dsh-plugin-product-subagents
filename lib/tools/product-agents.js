...
import { stripUndefined } from '../lossless.js'
import { childEntries } from '../children.js'

/** Overview tool: detected provider availability + live children. */
export function registerProductAgents(ctx, deps) {
  const { bindings, availability, runtimeName } = deps
    ...
    async execute(args, exec) {
      const availabilityView = Object.fromEntries(
        Object.entries(availability).map(([name, v]) => [name, {
          ...
          subagentProviderId: v.registered ? runtimeName(name) : undefined,
      ...
      try {
        ...
        for (const child of childEntries(list)) {
      ...
      return stripUndefined({ availability: availabilityView, children })
    },
  }))
}
